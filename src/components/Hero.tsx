...
export function Hero({ dict }: HeroProps) {
  ...
  const textY = useTransform(scrollYProgress, [0, 0.25], ["0vh", "-15vh"]);
  const textOpacity = useTransform(scrollYProgress, [0, 0.2], [1, 0]);

  // حركة اللاب توب (تتحرك وتستقر بسرعة وسلاسة عالية)
  const imageScale = useTransform(scrollYProgress, [0, 0.3], [0.85, 1]);
  const imageY = useTransform(scrollYProgress, [0, 0.3], ["20vh", "2vh"]);

  // محتوى النص المجمع (البادچ + العنوان + السطر الفرعي + زرار الـ CTA)
  const heroContent = (
    <>
      ...
      {/* زرار الـ CTA الأصلي زي ما كان مكتوب في كودك */}
  ...
  return (
    <section ref={containerRef} className="relative h-[90dvh] lg:h-[170vh] transition-colors duration-300">
      
      {/* Sticky Container */}
      <div className="sticky top-0 h-[100dvh] w-full overflow-hidden flex flex-col items-center justify-center lg:justify-start lg:pt-24">
        ...
        <motion.div
          className="hidden lg:block absolute z-30 w-full max-w-7xl px-2 sm:px-4 lg:px-8 mt-4"
